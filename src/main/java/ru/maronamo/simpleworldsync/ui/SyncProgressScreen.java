...
public final class SyncProgressScreen extends Screen {
    ...
    private final WorldEntry world;
    ...
    public SyncProgressScreen(Screen parent, WorldEntry world, SyncOperation operation) {
        super(Text.literal(operation.title()));
        this.parent = parent;
        this.world = world;
        this.operation = operation;
        this.syncService = SimpleWorldSyncClient.worldSyncService();
        this.progress = new SyncProgress(operation, world.displayName(), "Подготовка...", 0L, 0L, "");
    }
    ...
    private void startTask() {
        ...
        task = CancelableSyncTask.start(isCancelled -> switch (operation) {
            case UPLOAD -> SimpleWorldSyncClient.incrementalSyncService().incrementalUploadWorld(world.folderName(), world.folderName(), false, this::publishProgress, isCancelled);
            case RESTORE -> SimpleWorldSyncClient.incrementalSyncService().incrementalRestoreWorld(world.folderName(), this::publishProgress, isCancelled);
        });
        ...
        task.future().whenComplete((syncResult, throwable) -> {
            ...
            client.execute(() -> {
                if (throwable != null) {
                    SimpleWorldSyncClient.LOGGER.error("World sync operation failed", throwable);
                    result = SyncResult.error("Ошибка операции: " + friendlyMessage(throwable));
                } else {
                    result = syncResult;
                }

                if (parent instanceof SimpleWorldSyncScreen syncScreen) {
                    syncScreen.updateAfterOperation(result);
                }
